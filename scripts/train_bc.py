...
def main() -> int:
    ...
    # Load data.
    demo_paths = sorted(glob(cfg.demo_glob))
    if not demo_paths:
        log.error("no pickled demos matched %s", cfg.demo_glob)
    ...
    for epoch in range(cfg.epochs):
        ...
        log.info(
            "epoch %d: total=%.4f steering=%.4f buttons=%.4f grad_norm=%.6f",
            epoch + 1,
            stats.loss_total,
            stats.loss_steering,
            stats.loss_buttons,
        ...
        if (epoch + 1) % cfg.checkpoint_every == 0:
            ckpt = cfg.log_dir / f"bc_epoch_{epoch + 1}.pt"
            torch.save(
                {"model": model.state_dict(), "config": cfg.__dict__, "epoch": epoch + 1},
                ckpt,
            )
            log.info("wrote checkpoint %s", ckpt)
    ...
    logger.close()
    log.info("training done. best total loss: %.4f", best_loss)
    return 0
