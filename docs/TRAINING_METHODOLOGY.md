...
- Recurrent DQN with PER is known territory (R2D2 pattern). Replay buffer must either (a) store burn-in sequences long enough for the LSTM to warm up, or (b) store saved LSTM states per transition. **2026-04-23 update**: (a) was our initial choice and was abandoned due to activation-memory blowup — `bs=128 × seq_len=60` needed ~25 GB GPU even with bf16 + gradient checkpointing. We now use (b) — each replay transition carries the `(h, c)` the rollout agent consumed; `learn_step` forwards a single timestep with that stored hidden as the LSTM init. Kapturowski 2019 §3.2 ("stored state" variant). See `CHANGES.md` Phase 2.5 for the full rationale.
...
- ~~**LSTM state at replay time**. Start with burn-in approach. If memory footprint is a problem, fall back to stored-hidden-state approach (R2D2 style).~~ **Resolved 2026-04-23**: fell back to stored-hidden; see §2 and `CHANGES.md` Phase 2.5.
...
9. ~~**Sequence-level priority aggregation** per R2D2 §2.3 eq. 1: `η·max_t|δ_t| + (1-η)·mean_t|δ_t|` with `η=0.9`. VIPTankz is feed-forward so they update transition-level priorities directly; our LSTM variant needs this aggregation.~~ **Removed 2026-04-23** alongside the R2D2 sequence sampler; our stored-hidden replay samples single transitions so VIPTankz's transition-level priority path is what we use now.
...
12. **Target net noise permanently disabled** — VIPTankz resamples target-net noise every learn step (`BTR.py:990`). We instead call `disable_noise()` on the target at build and after every `sync_target`. This keeps the Munchausen `π_target = softmax(Q_target/τ)` deterministic within a batch, which is arguably more correct for the entropy-regularized Bellman target (Vieillard 2020 doesn't specify target-noise behavior). Revisit if A/B shows VIPTankz's noisy target helps exploration.
13. **Scalar `γ^n_step` bootstrap discount** — when an n-step window terminates early (via `trun` without setting `done`), the effective n is less than `n_step`, and applying a static `γ^n_step` to the bootstrap over-discounts. The bootstrap is zeroed on true `done` (via `(~dones)` mask), so only truncation-clipped windows have this bias. Impact is small at `n_step=3` and MKWii's ~1000-frame episodes; matches VIPTankz. Fix would require returning an effective-n from the replay sampler.
14. ~~**R2D2 target burn-in on `n_states[:, :burn_in]`** (not `states[:, :burn_in]`) — the target's learning-window forward consumes `n_states[:, burn_in:]`, so burning in on `n_states[:, :burn_in]` produces a hidden aligned to the first learning-window target input. R2D2 (Kapturowski 2019) doesn't address `n_step>1` explicitly; VIPTankz is feed-forward so sidesteps the question. Our choice is the only one that makes the target LSTM's hidden state consistent with its input sequence.~~ **Obsoleted 2026-04-23**: burn-in removed. Target now uses zero-init hidden at each learn_step (Kapturowski 2019 §3.2 "zero state"). Online net uses the stored-hidden path.
15. **Encoder Linear bottleneck (`flat_dim → feature_dim=256`) before the LSTM** — VIPTankz has no such bottleneck; their IQN cos-embedding mixes directly against the flat 2304-dim conv features (at `model_size=2`). v2 adds this bottleneck so the LSTM's `input_size` matches `feature_dim`, keeping LSTM parameter count tractable (~1.6M vs ~12M without bottleneck at 2304-dim input).
